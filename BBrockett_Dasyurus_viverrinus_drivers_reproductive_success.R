...
binmod_pruned<-glmmTMB(breedingsuccess ~ (scale(weight)+I((scale(weight))^2))*scale(age)+scale(indhet)+sex+morph+(1|id),
                       family="binomial", 
                       data=pruned, 
                       na.action=na.fail) 
...
plotweightbreed <- {ggplot(data = plotdat, 
    ...
    geom_line(linetype = "solid", 
              colour = "green4",
              linewidth = 1) +
    # Add vertical line at optimal weight - if useful, here it isn't
    #geom_vline(xintercept = plotdat$bestweight[1], 
              # linetype = "dashed", 
              # colour = "grey41") +
    # Add text label for optimal weight
  #  annotate("text", 
           #  x = plotdat$bestweight[1] + 0.1, 
           #  y = 0.95, 
           #  label = paste0(round(plotdat$bestweight[1], 3), "kg"),
           #  angle = 0, 
           #  color = "gray41") +
    # Add axis labels
    xlab("Weight (kg)") +
...
psn_pruned<-glmmTMB(offspring~ (scale(weight)+I((scale(weight))^2))*scale(age)+scale(indhet)+
                      cohort+
                      cluster+
                      morph+
                      sex+
                    ...
                    ziformula = ~., 
                    na.action=na.fail)
check_overdispersion(psn_pruned)
check_zeroinflation(psn_pruned)
check_model(psn_pruned) 
summary(psn_pruned)

psn_pruned2<-glmmTMB(offspring~ 
                       scale(weight)+
                       scale(age)+
                       scale(indhet)+
                       morph+
                     ...
                     ziformula = ~., 
                     na.action=na.fail)
check_overdispersion(psn_pruned2)
check_zeroinflation(psn_pruned2)
check_model(psn_pruned2) 
summary(psn_pruned2)
...
# because weight is so often not known, and thefore the dataset is quite limited, 
# try dataset where weight isn't a factor (noting age and weightlikely linked)
test <- subset(dat, age!="NA")
test <- subset(test, offspring!="NA")
test <- subset(test, morph!="NA")
test <- subset(test, age!="NA")
test <- subset(test, year!="NA")
test <- subset(test, sex!="NA")
test <- subset(test, indhet!="NA")
test <- subset(test, cohort!="NA")
test <- subset(test, cluster!="NA")
test <- subset(test, id!="88C7E")

psn_test<-glmmTMB(offspring~scale(age)+cluster+morph+sex+(1|id)+(1|year),
                  data=test,
                  family="poisson",
                  na.action=na.fail)
check_overdispersion(psn_test)
check_model(psn_test) # variance not homogenous, slightly overdispersed at high numbers
summary(psn_test)

# proceed with psn_test because this model has best fit and normality of residuals 
psnsel<-dredge(psn_test)
...
# plot ####
plot_model(topmod_psn, type="pred", terms="age[all]")

ages <- seq(min(test$age), max(test$age), length.out = 100)
newdata <- data.frame(age = ages)
newdata$morph <- factor(rep(levels(test$morph)[1], nrow(newdata)), 
                        levels = levels(test$morph))

newdata$predicted <- predict(topmod_psn, 
                             newdata = newdata, 
                             type = "response", 
                             re.form = NA)
preds_link <- predict(topmod_psn, 
                      newdata = newdata, 
                      se.fit = TRUE, 
                      re.form = NA)
pred_fit <- preds_link$fit
pred_se <- preds_link$se.fit
link_lower <- pred_fit - 1.96 * pred_se
link_upper <- pred_fit + 1.96 * pred_se
newdata$conf.low <- exp(link_lower)
newdata$conf.high <- exp(link_upper)

threshold <- 0.05
zero_offspring_index <- which(newdata$predicted <= threshold)[1]


age_effect_plot <- ggplot(newdata,
                          aes(x = age, 
                              y = predicted)) +
  ...
  geom_ribbon(aes(ymin = conf.low,
              ...
              alpha = 0.2, 
              fill = "darkorchid") +
  geom_vline(xintercept = zero_offspring_age, 
             linetype = "dashed", 
             color = "grey41") +
  annotate("text", 
           x = zero_offspring_age + 0.5,
           y = max(newdata$predicted) * 0.5,  # Position at half of max value for visibility
           label = paste0("≈ 0 offspring at\n", round(zero_offspring_age, 1), " years"),
           color = "gray41") +
  labs(x = "Age",
       y = "Predicted Number of Offspring",
       title = "Effect of Age on Reproductive Output") +
  theme_classic() +
  scale_y_continuous(limits = c(0, NA))  # Force y-axis to start at 0

age_effect_plot
ggsave("reproductive_output_by_age.png", age_effect_plot, width = 8, height = 6, dpi = 300)
...
morph_data <- data.frame(
  morph = factor(levels(test$morph), levels = levels(test$morph)),
  age = rep(specific_age, length(levels(test$morph))))

morph_data$predicted <- predict(topmod_psn, 
                                newdata = morph_data, 
                                type = "response", 
                                re.form = NA)
...
preds_link <- predict(topmod_psn, 
                      ...
                      se.fit = TRUE, 
                      re.form = NA)
link_lower <- preds_link$fit - 1.96 * preds_link$se.fit
link_upper <- preds_link$fit + 1.96 * preds_link$se.fit
morph_data$conf.low <- exp(link_lower)
morph_data$conf.high <- exp(link_upper)

# Extract random effects variance to estimate total variation
rand_eff_var <- VarCorr(topmod_psn)
id_var <- rand_eff_var$cond$id[1] # Variance for ID random effect
year_var <- rand_eff_var$cond$year[1] # Variance for year random effect

observed_data <- test[test$age >= 0 & test$age <= 4, ]

morph_effect_plot <- ggplot() +
  geom_bar(data = morph_data,
           aes(x = morph, y = predicted, fill = morph),
           stat = "identity", width = 0.6) +
  geom_errorbar(data = morph_data,
                aes(x = morph, ymin = conf.low, ymax = conf.high),
                width = 0.2) +
  geom_jitter(data = observed_data,
              aes(x = morph, 
                  y = offspring, 
                  color = morph),
              width = 0.2,
              ...
              alpha = 0.5,
              size = 2) +
  scale_fill_manual(values = c("grey25", "sandybrown")) +
  scale_color_manual(values = c("black", "tan3")) +
  labs(x = "Morph", 
       y = "Expected Mean Number of Offspring",
       title = "Effect of Morph on Reproductive Output") +
  theme_classic()+
  guides(color = guide_legend(title = "Observations"),
         fill = guide_legend(title = "Model Prediction"))
morph_effect_plot

ggsave("morph_effect_on_repro_output.png", morph_effect_plot, width = 8, height = 6, dpi = 300)
...
binmod_f_avg <- model.avg(modsel_f)
...
f_weight_plot <- {ggplot(plotdat_f, 
                         aes(x = weight_kg, 
                             y = fit,
                             xmin=min(females$weight),
                             xmax=max(females$weight))) +
    geom_line(size = 1) +
    geom_ribbon(aes(ymin = lower, 
                    ymax = upper), 
                alpha = 0.6, 
    ...
    theme(axis.line = element_line(colour = "black"),
          ...
          plot.margin = margin(0.5, 0.5, 0.5, 0.5, "cm"),
          plot.title = element_text(face = "bold", size = 12),
          text = element_text(size = 12, colour = "black", face = "plain"))}
f_weight_plot
